...
def extra_clean( files ):
    # TODO. Replace calls to extra_clean by appropriate calls to extra_files.
    return lambda _name, _opts: None
...
def _newTestDir(name, opts, tempdir, dir):
    opts.srcdir = os.getcwd() + '/' + dir
    opts.testdir = tempdir + '/' + dir + '/' + name
...
source_suffixes = ['hs', 'lhs', 'hsc', 'script', 'hs-boot', 'hsig']

def source_files(name):
    return [add_suffix(name, suffix) for suffix in source_suffixes]
...
def do_test(name, way, func, args):
    ...
    try:
        ...
        # Link all source files for this test into a new directory, and
        # run the test in that directory. This makes it possible to run
        # tests concurrently, without modification, that would otherwise
        # (accidentally) write to the same output file, and it makes
        # it easier to keep the testsuite clean.
        os.makedirs(opts.testdir)

        if opts.extra_files:
            # New API.
            for filename in set(source_files(name) + opts.extra_files):
                ...
                if os.path.isfile(src):
                    ...
                    os.mkdir(dst)
                    os.system('lndir -silent {} {}'.format(src, dst))
        else:
            # Old API. TODO. This is rather slow (but it works, sort of).
            os.system('lndir -silent {} {}'.format(opts.srcdir, opts.testdir))

        # TODO. This is rather hackish (but it works, sort of).
        ...
        if os.path.exists(src_makefile):
            with open(src_makefile, 'r') as src:
                content = src.read().replace(
                    'TOP=', 'TOP={}/'.format(opts.srcdir), 1)
                # The lndir created a link to dst_makefile already.
                # Delete it first.
...
def check_prof_ok(name, way):
    ...
    expected_prof_path = in_srcdir(expected_prof_file)
